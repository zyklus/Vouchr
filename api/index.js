var        config = require('./../config'),
   ...
   validateParams = require('./validateParams'), // TODO: something with this file
          ...
          nullKey = 8907012380912378,
            cache = require('./cache'),
        cassandra = require('vouchr_cassandra');
...
var paramSteps = step.fn({
...
}, function step1(params, fn, apiName, data, cb, req, res, next){
	...
	for(var p in s.params){
		...
		outData[p] = s.data[p] ? (
			...
			(type == 'date')  ? s.data[p] instanceof Date ? s.data[p] : new Date(s.data[p] + ' GMT') :
			(type == 'json')  ?
	...
	// always pass in the auth token if it's there
	s.data.auth_token && (outData.auth_token = s.data.auth_token);
...
function onAPIResponse(cb, data, authToken, expiresAt, req, apiResp, isErr){
	if(data.external && (((req.params && req.params.format) || data.format) == 'jsonc')){
